...
#  7. Read merged ratings/features from data/preprocessed/8#Belt Conveyer_merged.csv
...
# 11. Save full features to data/preprocessed/8#Belt Conveyer_full_features.csv
# ---------------------------------------------------------------------------- #
...
def get_config():
    ...
    config['VOLTAGE_DIR'] = config['PROJECT_ROOT'] / "data" / "voltage"
    config['PROCESS_DIR'] = config['PROJECT_ROOT'] / "data" / "preprocessed"

    return config


# —————————— DSP PROCESSING FUNCTIONS ——————————
def read_json_file(path: Path = None, content: str = None) -> tuple[float, np.ndarray]:
...
def band_rms(sig: np.ndarray, fs: float, f_lo: float, f_hi: float) -> float:
    """Calculate RMS value in frequency band"""
    n = sig.size
...
def band_peak(sig: np.ndarray, fs: float, f_lo: float, f_hi: float) -> float:
    """Calculate peak value in frequency band"""
    n = sig.size
    X = np.fft.rfft(sig - sig.mean())
    freqs = np.fft.rfftfreq(n, d=1.0 / fs)
    mask = (freqs >= f_lo) & (freqs <= f_hi)
...
def compute_dsp_metrics(w: np.ndarray, fs: float) -> list[float]:
    """Compute all DSP metrics for a waveform"""
    vrs = np.sqrt(np.mean(w ** 2))
    pk = np.max(np.abs(w))

    return [
        vrs,  # velocity_rms
        pk / vrs if vrs > 0 else np.nan,  # crest_factor
        stats.kurtosis(w, fisher=False),  # kurtosis_opt
        pk,  # peak_value_opt
        band_rms(w, fs, 0.1, 10),  # rms_0_10hz
        band_rms(w, fs, 10, 100),  # rms_10_100hz
        band_rms(w, fs, 1000, 10000),  # rms_1_10khz
        band_rms(w, fs, 10000, 25000),  # rms_10_25khz
        band_peak(w, fs, 10, 1000),  # peak_10_1000hz
    ]


# —————————— BUCKET PROCESSING FUNCTIONS ——————————
def bucket_summary(
        df: pd.DataFrame,
    ...
    """Create bucket summary of measurements"""
    df = df.copy()
...
def collect_json_files(aws_mode=False, device_name="8#Belt Conveyer"):
    ...
    if aws_mode:
        print(f"Reading JSON files from S3 bucket: {config['S3_BUCKET']}")
        s3 = boto3.client('s3')
        prefix = 'voltage/'
        ...
        print(f"Reading JSON files from local directory: {config['VOLTAGE_DIR']}")
        for subdir in config['VOLTAGE_DIR'].iterdir():
            ...
            for root, _, files in os.walk(subdir):
                for fn in files:
                    ...
                    rec = {"timestamp": ts,
                           "filepath": str(Path(root) / fn).split(str(config['PROJECT_ROOT']) + os.sep)[1]}
                    for col in config['METRIC_COLS']:
                        rec[col] = pd.NA
                    records.append(rec)
...
def read_merged_csv(device_name, aws_mode=False):
    ...
    if aws_mode:
        print(f"Reading merged CSV from S3 bucket: {config['S3_BUCKET']}")
        s3 = boto3.client('s3')
        key = f"preprocessed/{device_name}_merged.csv"
        obj = s3.get_object(Bucket=config['S3_BUCKET'], Key=key)
        return pd.read_csv(io.BytesIO(obj['Body'].read()), parse_dates=["datetime"])
    else:
        merged_path = config['PROCESS_DIR'] / f"{device_name}_merged.csv"
...
def save_csv(df, filename, aws_mode=False, s3_prefix=''):
    ...
    if aws_mode:
        ...
        s3.put_object(Bucket=config['S3_BUCKET'], Key=key, Body=csv_buffer.getvalue())
        print(f"Saved {filename} to s3://{config['S3_BUCKET']}/{key}")
    else:
        if s3_prefix == 'voltage':
            output_path = config['VOLTAGE_DIR'] / filename
        else:
            output_path = config['PROCESS_DIR'] / filename
        df.to_csv(output_path, index=False)
        print(f"Saved {filename} to {output_path}")


# —————————— METRICS PROCESSING FUNCTIONS ——————————
def process_json_metrics(aws_mode=False, device_name="8#Belt Conveyer"):
    """Process JSON files and compute DSP metrics"""
    config = get_config()
    ...
    extracted = metrics_df['file_name'].str.extract(config['FILENAME_PATTERN'])
    metrics_df['sensor_id'] = extracted[0]
    metrics_df['wave_code'] = extracted[1]
    metrics_df['location'] = metrics_df['sensor_id'].map(config['SENSOR_MAP'])
    ...
    for i, row in metrics_df.iterrows():
        try:
            if aws_mode:
                obj = s3.get_object(Bucket=config['S3_BUCKET'], Key=row['s3_key'])
                content = obj['Body'].read().decode('utf-8')
                fs, w = read_json_file(content=content)
            else:
                p = config['PROJECT_ROOT'] / row['filepath']
                fs, w = read_json_file(path=p)
        except Exception as e:
            print(f"Error processing file {row['filepath']}: {e}")
            continue

        vals = compute_dsp_metrics(w, fs)
        for j, col in enumerate(config['METRIC_COLS']):
            metrics_df.at[i, col] = vals[j]

    return metrics_df


def process_merged_data(device_name, aws_mode=False):
    """Process merged data and create bucket summary"""
    config = get_config()
    ...
    merged_df = merged_df.rename(columns={c: f"{c}_rating" for c in config['RATING_COLS']})
    ...
    rating_cols_renamed = [f"{c}_rating" for c in config['RATING_COLS']]
...
def merge_metrics_and_summary(metrics_df: pd.DataFrame, summary_df: pd.DataFrame) -> pd.DataFrame:
    """Merge metrics and summary dataframes using interval-based lookup"""
...
def run_feature_engineering_pipeline(device_name="8#Belt Conveyer", aws_mode=False):
    ...
    metrics_df = process_json_metrics(aws_mode, device_name)

    # save JSON metrics
    save_csv(metrics_df, "metrics_json.csv", aws_mode, "voltage")

    # —————————— PROCESS MERGED DATA ——————————
    print("Processing merged data and creating bucket summary...")
    summary_df = process_merged_data(device_name, aws_mode)

    # save bucket summary
    save_csv(summary_df, f"{device_name}_bucket_summary.csv", aws_mode, "preprocessed")

    # —————————— MERGE METRICS + SUMMARY VIA INTERVAL-INDEX LOOKUP ——————————
    print("Merging metrics and summary data...")
    full_df = merge_metrics_and_summary(metrics_df, summary_df)

    # Save final results
    save_csv(full_df, f"{device_name}_full_features.csv", aws_mode, "preprocessed")
...
def main(device_name="8#Belt Conveyer", aws_mode=False):
    """Main function that maintains original interface"""
    return run_feature_engineering_pipeline(device_name, aws_mode)
