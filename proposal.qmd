...
- We perform EDA to extract meaningful insights from both the input sensor features and the output device ratings.
  - For input features, we analyze distributions, trends, and correlations among the four sensor parameters (vibration, acceleration, temperature) to understand equipment behavior and identify anomalies or patterns.
  - For output device ratings, we examine the range, frequency, and temporal changes of the 15 device health metrics, looking for trends, outliers, and relationships with sensor data.
- Visualization techniques and summary statistics are used to reveal key relationships and guide feature engineering.
